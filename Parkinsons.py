...
fig, ax = plt.subplots()
scatPlot = ax.scatter(df["HNR"], df["NHR"], s=1)
...
for rowInd in range(0, 195):
    ...
    manhatBlockDistances.append(
        (abs(P[0] - dataArr[rowInd][0]) + abs(P[1] - dataArr[rowInd][1]))
    )

    minkowDistances.append(
        (abs(dataArr[rowInd][0] - P[0]) ** 6 + abs(dataArr[rowInd][1] - P[1]) ** 6)
        ** (1 / 6)
    )

    chebyDistances.append(
        max(abs(P[0] - dataArr[rowInd][0]), abs(P[1] - dataArr[rowInd][1]))
    )

    dotProd = (P[0] * dataArr[rowInd][0]) + (P[1] * dataArr[rowInd][1])
    d1 = ((P[0] * P[0]) + (P[1] * P[1])) ** 0.5
    d2 = (
        (dataArr[rowInd][0] * dataArr[rowInd][0])
        + (dataArr[rowInd][1] * dataArr[rowInd][1])
    ) ** 0.5
...
for index in range(0, 5):
    print(
        "\t(",
        np.round(dataArr[euclMins[index]][0], 5),
        ",",
...
for index in range(0, 5):
    print(
        "\t(",
        np.round(dataArr[manhatMins[index]][0], 5),
        ",",
...
for index in range(0, 5):
    print(
        "\t(",
        np.round(dataArr[minkowMins[index]][0], 5),
        ",",
...
for index in range(0, 5):
    print(
        "\t(",
        np.round(dataArr[chebyMins[index]][0], 5),
        ",",
...
for index in range(0, 5):
    print(
        "\t(",
        np.round(dataArr[cosineMins[index]][0], 5),
        ",",
